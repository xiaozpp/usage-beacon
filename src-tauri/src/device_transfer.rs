    ...
    db.with_conn(|conn| {
        ...
        let mut stmt = conn.prepare(
            "SELECT request_id, provider_id, app_type, model, request_model, pricing_model,
                    ...
                    data_source, device_id, device_name
             FROM proxy_request_logs
        ...
        let rows = stmt.query_map([], |row| {
            Ok(TransferRecord {
                ...
                provider_type: row.get(22)?,
                is_streaming: row.get(23)?,
                cost_multiplier: row.get(24)?,
                created_at: row.get(25)?,
                data_source: row.get(26)?,
                device_id: row.get(27)?,
...
pub fn import_usage_data(db: &Database, contents: &str) -> Result<UsageImportResult> {
    ...
    db.with_conn(|conn| {
        ...
        let mut device_ids = BTreeSet::new();
        {
            ...
            for mut record in transfer.records {
                if record.device_id.trim().is_empty() {
                    record.device_id = transfer.source_device.id.clone();
                }
                if record.device_name.trim().is_empty() {
                    record.device_name = transfer.source_device.name.clone();
                }
